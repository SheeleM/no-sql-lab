...
router.get('/consulta1', async (req, res) => {
  try {
    let filters = {};
   filters =   { publicados: { $lte: 20 } ,pais: { $eq: "Colombia"}}; //{ publicados: req.query.publicados <=20}; 
...
router.get('/consulta2', async (req, res) => {
  try {
    let filters = {};
   filters =   {apellido: {$ne: null} } //filters = {apellido: {$ne:null}}; 
...
router.get('/consulta3', async (req, res) => {
  try {
    let filters = {};
   filters = {$or:[{publicados: {$gt: 20}} ,{pais: {$eq: "Argentina"}}]}  //db.inventory.find( { $or: [ { quantity: { $lt: 20 } }, { price: 10 } ] } )
